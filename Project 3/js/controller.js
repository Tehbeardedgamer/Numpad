...
function showNumpad() {
  ...
  window.addEventListener("keydown", function (event) {
    ...
    if (event.which === 8) setCode(code.substr(0, code.length - 1));
  });
}
function postlogin(data /* , settings */) {
  // אחרי כוללאזאקס מביא לי את הנתונים ואת כל הפרמטרים כדי להתחבר לשרת
  if (data.affectedRows == 1) {
    // אם המשתנה של שינוי טבלה ברשת נעשה שווה ל1 אז תכנס
    View.welcomeUser();
    //getInfo(settings.data.username); // תלך לשרת ותביא לי את כל הנתונים החדשים
  } else {
    // אם בשרת זה לא שינה תכנס לכאן
...
//      y++;
//      callAFunction( RUN CHANGE COLOR FUNCTION);
//    } else {
//      setTimeout(function () {
//        x++;
//      }, 3000);
//      doSomethingElse( CLEAR INPUT FIELD AND RESET COLORS BACK TO DEFAULT); document.getQuerySelectorAll('button') > change color back to black.
//    }
//    finalOperation();
//  }
